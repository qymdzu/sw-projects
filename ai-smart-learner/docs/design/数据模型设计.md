...
| `grade` | TEXT | | `'primary_1' ~ 'primary_6'` / `'junior_7' ~ 'junior_9'` / `'senior_10' ~ 'senior_12'`（仅学生，v0.1 简化：仅影响主题色）|
...
| `subject` | TEXT | NOT NULL | "数学" / "语文" / "英语" / "物理" / "化学" / "生物" / "政治" / "历史" / "地理" |
| `stage` | TEXT | NOT NULL | `'primary'` / `'junior'` / `'senior'`（学段，公子 2026-06-06 补充）|
...
**预置数据**（v0.1 启动时初始化小学数学/语文/英语，v0.2 推初/高中）：

```json
// assets/chapters/math_primary.json
[
  {"id": "math-p-1", "subject": "数学", "stage": "primary", "name": "数的认识", "parent_id": null, "order": 1},
  {"id": "math-p-1-1", "subject": "数学", "stage": "primary", "name": "10 以内加减", "parent_id": "math-p-1", "order": 1},
  ...
]

// v0.2 推
// assets/chapters/math_junior.json
// assets/chapters/math_senior.json
```
